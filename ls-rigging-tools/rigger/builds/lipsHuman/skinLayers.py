...
import rigger.utils.weighting.lip_weighting as lip
reload(lip)

import rigger.utils.weighting.loop_weighting as loop_weighting
reload(loop_weighting)

def splitLipsVertices(mll):
    '''
    returns two lists for vertIds
    upLips and lowLips
    '''
    meshName, sknName = mll.getTargetInfo()
    mesh = pm.PyNode(meshName)
    vertCount = mll.getVertCount()
    
    # deal with ambiguous regions first
    # (area just around lips)
    pGrp = pm.PyNode('CT_placement_grp')
    treeRoot = lip.getLipVertexLoops(pGrp, 6)
    ...
    for bnd, vertsList in bndToVertsIdsMap.items():
        if '_upper' in bnd:
            upLipsVerts += vertsList
        elif '_lower' in bnd:
            lowLipsVerts += vertsList
        elif '_corner' in bnd:
            upLipsVerts += vertsList
            lowLipsVerts += vertsList
        else:
            pm.warning('Unknown bnd '+bnd)
    
    # deal with unambiguous regions
    # just put into up or low based on position.y
    # get y-height from corner bnd
    cornerBnd = pm.PyNode('LT_corner_lip_bnd')
    yHeight = cornerBnd.getTranslation(space='world').y
    for vertId in range(vertCount):
        if vertId not in upLipsVerts and vertId not in lowLipsVerts:
            vert = mesh.vtx[vertId]
            if vert.getPosition().y > yHeight:
                upLipsVerts.append(vertId)
            else:
                lowLipsVerts.append(vertId)

    return upLipsVerts, lowLipsVerts
...
def maskWeightsByVertsList(weightsList, maskVerts):
    '''
    '''
    for vertId in range(len(weightsList)):
        if vertId not in maskVerts:
            weightsList[vertId] = 0
    return weightsList

    
def layer_chin(mll, inMaskVerts):
    '''
    '''
    mesh, skn = mll.getTargetInfo()
    mesh = pm.PyNode(mesh)
    jntInfs = [u'LT_low_crease_bnd',
                u'LT_mid_chin_bnd',
                u'CT_mid_chin_bnd']
    ...
    layerInfo = 'Chin', jntInfs, jntInfs, maskInfs
...
def layer_cheeks(mll, inMaskVerts):
    '''
    '''
    mesh, skn = mll.getTargetInfo()
    mesh = pm.PyNode(mesh)
